...
impl Database {
    ...
    fn finalize_labels(
        input_node_labels: HashMap<InputNodeLabel, DatabaseLabelCounts>,
        input_edge_labels: HashMap<CombinedInputLabel, DatabaseLabelCounts>,
    ) -> (Vec<DatabaseNodeLabel>, Vec<DatabaseEdgeLabel>) {
        let edge_labels = input_edge_labels
            .into_iter()
            .map(|(label, counts)| {
                let (from_node_label, input_edge_label, to_node_label) = label;
                DatabaseEdgeLabel {
                    input_edge_label,
                    to_node_label: types::NodeLabel(
                        input_node_labels
                            .get(&to_node_label)
                            .expect("Input node labels should contain {to_node_label}")
                            .id as u8,
                    ),
                    from_node_label: types::NodeLabel(
                        input_node_labels
                            .get(&from_node_label)
                            .expect("Input node labels should contain {from_node_label}")
                            .id as u8,
                    ),
                    edge_label: todo!(),
        ...
        let mut node_labels = input_node_labels
            .into_iter()
            .map(|(label, counts)| DatabaseNodeLabel {
    ...
    fn prune_and_assign_ids<K: std::hash::Hash + Eq>(
        labels: &mut HashMap<K, DatabaseLabelCounts>,
    ...
    fn prune_infrequent_nodes_and_edges(
        mut trees: Vec<RawInputGraph>,
        ...
        trees
            ...
            .map(|(tid, tree)| {
                ...
                tree.edges
                    .retain(|e| edge_labels.contains_key(&Self::get_combined_label(e, nodes)));
                ...
                let mut new_tree = DatabaseTree {
                    ...
                    nodes: nodes
                        ...
                        .map(|(node, _)| {
                            DatabaseTreeNode::new(types::NodeLabel(
                                node_labels.get(&node.label).unwrap().id as u8,
                            ))
                        })
                        .collect(),
                };
                ...
                // Add the edges
                for edge in &tree.edges {
                    let edgelabel = types::EdgeLabel(
                        edge_labels
                            .get(&Self::get_combined_label(edge, nodes))
                            .unwrap()
                            .id as u8,
                    );
                    new_tree.nodes[edge.from.0 as usize]
                        .edges
                        .push(DatabaseTreeEdge {
                            edgelabel,
        ...
        for (line, line_no) in reader.lines().zip(1..) {
            match Self::read_command((line_no, &line?))? {
                ...
                Some(Command::Edge(id1, id2, label)) => {
                    let last_graph =
                        &mut trees.last_mut().ok_or(DatabaseError::InvalidFirstLine)?;
                    ...
                    last_graph.edges.push(RawInputEdge {
                        from: id1,
                        to: id2,
                        label,
                    });
    ...
    fn count_label<K: std::hash::Hash + Eq>(
        labels: &mut HashMap<K, DatabaseLabelCounts>,
        key: K,
        tid: usize,
    ) {
        let label = labels
            .entry(key)
            .or_insert(DatabaseLabelCounts::new(1, 0, tid));
    ...
    fn count_labels(
        ...
        HashMap<InputNodeLabel, DatabaseLabelCounts>,
        ...
        for (tid, tree) in trees.iter().enumerate() {
            ...
            for edge in &tree.edges {
                Self::count_label(
                    &mut edge_labels,
                    Self::get_combined_label(&edge, &tree.nodes),
                    tid,
                );
            }
        }
    ...
    fn read_command((line_no, line): (usize, &str)) -> Result<Option<Command>, DatabaseError> {
        ...
        match token_iterator.next() {
            Some("t") => {
                ...
                let tid = Self::parse_token(
                    &mut token_iterator,
                    line_no,
                    DatabaseError::IncompleteGraphCommand(line_no),
                )?;

                Ok(Some(Command::Graph(types::Tid(tid))))
            }
            Some("v") => {
                let id = Self::parse_token(
                    &mut token_iterator,
                    line_no,
                    DatabaseError::IncompleteNodeCommand(line_no),
                )?;

                let label = Self::parse_token(
                    &mut token_iterator,
                    line_no,
                    DatabaseError::IncompleteNodeCommand(line_no),
                )?;

                Ok(Some(Command::Node(InputNodeId(id), InputNodeLabel(label))))
            }
            Some("e") => {
                let nodeid1 = Self::parse_token(
                    &mut token_iterator,
                    line_no,
                    DatabaseError::IncompleteEdgeCommand(line_no),
                )?;

                let nodeid2 = Self::parse_token(
                    &mut token_iterator,
                    line_no,
                    DatabaseError::IncompleteEdgeCommand(line_no),
                )?;
...
mod tests {
    ...
    fn test_valid_commands() {
        ...
        assert_eq!(
            result.unwrap(),
            Some(Command::Node(InputNodeId(5), InputNodeLabel(6)))
        );
    ...
    fn test_incomplete_commands() {
        ...
        assert!(
            matches!(&err, DatabaseError::IncompleteGraphCommand(line_no)
                if line_no == &15
            )
        );
        ...
        assert_eq!(
            msg,
            "line 39: Incomplete edge, requires \"e [nodeid1] [nodeid2] [label]\""
        );
    }
    ...
    const TEST_INPUT_STRING: &str = "t # 0\n\
		 v 0 15\n\
		 v 1 4\n
		 e 1 0 2\n\
		 t # 1\n\
    ...
    fn test_parse_input() {
        ...
        assert_eq!(
            ...
            vec![RawInputGraph {
                nodes: vec![
                    RawInputNode {
                        label: InputNodeLabel(15)
                    },
                    RawInputNode {
                        label: InputNodeLabel(4)
                    },
                ],
                edges: vec![RawInputEdge {
                    from: InputNodeId(1),
                    to: InputNodeId(0),
                    label: InputEdgeLabel(2)
                },],
            },]
        );
        ...
        assert_eq!(
            ...
            vec![
                ...
                RawInputGraph {
                    nodes: vec![
                        RawInputNode {
                            label: InputNodeLabel(1)
                        },
                        RawInputNode {
                            label: InputNodeLabel(2)
                        },
                        RawInputNode {
                            label: InputNodeLabel(3)
                        },
                        RawInputNode {
                            label: InputNodeLabel(4)
                        },
                        RawInputNode {
                            label: InputNodeLabel(5)
                        },
                        RawInputNode {
                            label: InputNodeLabel(6)
                        },
                        RawInputNode {
                            label: InputNodeLabel(7)
                        },
                    ],
                    edges: vec![
                        RawInputEdge {
                        ...
                        RawInputEdge {
                            from: InputNodeId(3),
                            to: InputNodeId(4),
                            label: InputEdgeLabel(5)
                        },
                        RawInputEdge {
                            from: InputNodeId(4),
                            to: InputNodeId(5),
                            label: InputEdgeLabel(6)
                        },
                        RawInputEdge {
                            from: InputNodeId(5),
                            to: InputNodeId(6),
                            label: InputEdgeLabel(7)
                        },
                    ],
                },
            ]
